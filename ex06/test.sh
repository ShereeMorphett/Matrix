...
if [ "$1" == "clean" ]; then
    ...
    if [ -f ex06_test ]; then
        rm ex06_test
        print_color "cyan" "Deleted compiled executable 'ex06_test'."
    else
        print_color "yellow" "No compiled executable found ('ex06_test')."
    fi
    exit 0
fi

# Compile the C++ code with ex06.cpp
g++ -o ex06_test ex06.cpp -I.
