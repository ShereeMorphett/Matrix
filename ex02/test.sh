...
if [ "$1" == "clean" ]; then
    ...
    if [ -f ex02_test ]; then
        rm ex02_test
        print_color "cyan" "Deleted compiled executable 'ex02_test'."
    else
        print_color "yellow" "No compiled executable found ('ex02_test')."
    fi
    exit 0
fi

# Compile the C++ code with lerp function
g++ -o ex02_test ex02.cpp -I.
